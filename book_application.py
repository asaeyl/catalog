...
from database_setup import Base ,Categories ,User
...
def check_admin():
    return session.query(User).filter_by(
        email='asaeyl202@gmail.com').one_or_none()
...
def newBook():
    if request.method == 'POST':
        ...
        if 'provider' in login_session and \
            ...
            if bookName and bookAuthor and description \
                    and bookCategory:
                newBook = Categories(
                    bookName=bookName,
...
def sortBooks(category):
    books = session.query(Categories).filter_by(category=category).all()
    state = new_state()
    return render_template(
        'main.html',
        books=books,
...
def bookDetail(category, bookId):
    book = session.query(Categories).filter_by(id=bookId,
...
def deleteBook(category, bookId):
    book = session.query(Categories).filter_by(category=category,
                                           id=bookId).first()
    state = new_state()
    if book:
        # check if user is logged in or not
...
def booksJSON():
    books = session.query(Categories).all()
